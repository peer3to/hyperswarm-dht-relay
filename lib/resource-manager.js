...
class ResourceManager {
  constructor (options = {}) {
    ...
    // Track resource usage per client ID
    // Map<clientId: string, {
  ...
  /**
   * Check if a new connection is allowed for this client
   */
  checkConnection (clientId) {
    const client = this._ensureClient(clientId)
  ...
  checkDataTransfer (connectionAlias, additionalBytes) {
    ...
    const client = this._ensureClient(connData.clientId)
  ...
  recordConnection (clientId, connectionAlias = null) {
    const client = this._ensureClient(clientId)
    client.connections++
  ...
  recordDisconnection (clientId, connectionAlias = null) {
    const client = this._ensureClient(clientId)
  ...
  checkMessage (_clientId, messageType, _messageData = null) {
   ...
   */
  getUsage (clientId) {
    return this._ensureClient(clientId)
  }
  ...
  getAllConnectionData () {
    ...
    for (const [alias, data] of this.connectionData.entries()) {
      stats[alias] = { ...data, clientId: data.clientId }
    }
    return stats
  }
