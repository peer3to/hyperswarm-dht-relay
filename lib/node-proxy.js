...
class NodeProxy {
  constructor (dht, protocol, defaultKeyPair, resourceManagerOptions = {}) {
    this._dht = dht
    this._protocol = protocol
    this._defaultKeyPair = defaultKeyPair
    this._resourceManager = new ResourceManager(resourceManagerOptions)
...
function onConnect (message) {
  ...
  const connectionCheck = this._resourceManager.checkConnection(message.publicKey)
  ...
  this._resourceManager.recordConnection(message.publicKey, remoteAlias)
  ...
  const onError = (err) => {
    this._resourceManager.recordDisconnection(message.publicKey, remoteAlias)
  ...
  const onData = (data) => {
    // Forward incoming data from peer to client without limits
...
function onData (message) {
  ...
  if (stream) {
    const dataCheck = this._resourceManager.canSendData(message.alias, message.data)
    if (!dataCheck.allowed) {
      console.log(`Outgoing data transfer denied for connection ${message.alias}: ${dataCheck.reason}`)
