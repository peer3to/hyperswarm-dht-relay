...
class NodeProxy {
  constructor(
    ...
    this._resourceManager = sharedResourceManager;

    this._identifierStrategy = resourceManagerOptions.identifierStrategy;
    this._ip = resourceManagerOptions.ip;

    this._identifierStrategies = {
      publicKey: (message) => message.publicKey.toString("hex"),
      ip: (_message) => this._ip,
      address: (message) => message.address,
      default: (message) => message.publicKey.toString("hex"),
    };
    ...
    this._protocol._stream.once("close", this._onStreamClose);
  ...
  _getClientIdentifier(message) {
    const strategy = this._identifierStrategies[this._identifierStrategy];
    return strategy
      ? strategy(message)
      : this._identifierStrategies.default(message);
  }
}
...
function onConnect(message) {
  ...
  console.log(
    `DEBUG: Connection request received for message.alias: "${message.alias}"`
  );

  const alias = nextId();

  const clientId = this._getClientIdentifier(message);

  const connectionCheck = this._resourceManager.checkConnection(clientId);

  if (!connectionCheck.allowed) {
    this._protocol.destroy.send({
      paired: false,
      remoteAlias,
      error: `Connection denied: ${connectionCheck.reason}`,
    });
    return;
  }

  this._resourceManager.recordConnection(clientId, remoteAlias);
  ...
  const stream = this._dht.connect(message.remotePublicKey, {
    ...
    createHandshake: custodial ? null : createHandshake.bind(this, remoteAlias),
  ...
  const onError = (err) => {
    this._resourceManager.recordDisconnection(clientId, remoteAlias);
...
function onData(message) {
  ...
  if (stream) {
    const dataCheck = this._resourceManager.canSendData(
      message.alias,
      message.data
    );
    if (!dataCheck.allowed) {
      console.log(
        `Outgoing data transfer denied for connection ${message.alias}: ${dataCheck.reason}`
      );
      stream.destroy(new Error(`Data limit exceeded: ${dataCheck.reason}`));
      return;
    }

    this._resourceManager.recordDataSent(message.alias, message.data);
...
function onQuery(message, query, encoding) {
  ...
  const onClose = () => {
    query.off("error", onError).off("data", onData);
  ...
  query.once("error", onError).once("close", onClose).on("data", onData);
}

function onLookup(message) {
  this._onQuery(message, this._dht.lookup(message.topic), announcers);
}
