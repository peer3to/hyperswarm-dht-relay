...
function onConnect (message) {
  ...
  // Check if this connection is allowed by the resource manager
  const connectionCheck = this._resourceManager.checkConnection(message.publicKey)
  if (!connectionCheck.allowed) {
    // Reject the connection by sending destroy message
  ...
  const onData = (data) => {
    ...
    // (incoming data is not generated by the client, so no need to limit it)
...
function onData (message) {
  ...
  if (stream) {
    // Check if data can be sent (ResourceManager handles all the logic)
    ...
    if (!dataCheck.allowed) {
      ...
      // Close the connection for limit violation
    ...
    // Forward the data to the DHT peer
